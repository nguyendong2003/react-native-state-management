...
export default function Filters() {
  ...
  const filterPriorities = useSelector((state: RootState) => state.filters.priorities);
  ...
  const handlePriorityToggle = (priority: 'High' | 'Medium' | 'Low') => {
    ...
    if (filterPriorities.includes(priority)) {
      newPriorities = filterPriorities.filter((p) => p !== priority);
    } else {
      newPriorities = [...filterPriorities, priority];
    }
    dispatch(prioritiesFilterChange(newPriorities));
  };
